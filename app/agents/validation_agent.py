...
class ValidationAgent:
    ...
    def _hard_gates(result: dict, checks: list[dict]) -> list[QCFinding]:
        ...
        material_sections = ("authentication", "credential_access", "api", "mcp")
        evidence = []
        ...
        if not evidence:
            findings.append(QCFinding(severity="error", field="evidence", issue="No evidence records were supplied.", recommendation="Collect and attach evidence for material conclusions."))

        for item in evidence:
            url = str(item.get("url", ""))
            check = by_url.get(url)
            if check and not check["reachable"]:
                findings.append(QCFinding(severity="error", field="evidence", issue=f"Evidence URL is unreachable: {url}", recommendation="Replace it with a reachable source or mark the claim unknown."))
        ...
        if api.get("availability") == "public" and not api.get("evidence"):
            findings.append(QCFinding(severity="error", field="api", issue="Public API classification has no API evidence.", recommendation="Attach developer/API documentation or change the classification."))
        if creds.get("classification") == "self_serve" and not creds.get("evidence"):
            findings.append(QCFinding(severity="error", field="credential_access", issue="Self-serve credential classification has no credential-access evidence.", recommendation="Provide evidence of self-serve key/OAuth registration or use unknown/gated."))
